    ...
    <div class="login-container steps">
        ...
        <div class="signin-form email-form">
            ...
            <div class="under-email-signin clearfix">
                <div class="forgot-pass">
                    ...
                    {!!  trans('index.termslink', ['url' => '<a href="'.url('/s/kullanim-sartlari/').'" target="_blank">'.trans('index.terms').'</a>']) !!}
                </div>
            </div>
            <button type="button" class="button button-black button-full"  id="PostNewUser">{!! trans('index.register') !!}</button>
        ...
        <div class="signup-terms">
            <div class="show-connect-forms">
                {!! trans('index.Doyouhaveanaccount') !!} <a href="{{ action('Auth\AuthController@login') }}" @if(!isset($link)) rel="get:Loginform" @endif>{!! trans('index.login') !!}</a>
            </div>
        </div>
    </div>
