...
<div class="entry" data-type="quizquestion" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}" @endif>
    ...
    <h3><i class="fa fa-question-circle"></i> {!! trans('quiz.question') !!}</h3>
    <div class="inpunting  ordering">
        <button class="order-number button button-gray show">1</button>
        {!! Form::text(null, isset($entry->title) ? $entry->title : null ?: null, ['data-type' => 'title', 'class' => 'cd-input ', 'placeholder' => trans('quiz.entry_question')]) !!}
    </div>
    ...
    </div>{!! Form::hidden(null, isset($entry->image) ? makepreview($entry->image, null, 'entries') : null, ['data-type' => 'image', 'class' => 'cd-input-image ']) !!}
    ...
    <div class="moredetail questionmore">
        ...
        <div class="clear"></div>
        <a class="button button-white button-full submit-button postable answerbutton" data-method="Get" data-target="answer{{ $uniquid }}" data-puttype="append" data-type="answerform" href="{{ action('FormController@addnewform') }}?addnew=answer"><i class="fa fa-plus-circle"></i>{!! trans('addpost.add', ['type' => trans('quiz.answer')]) !!}</a>
    </div>
</div>
