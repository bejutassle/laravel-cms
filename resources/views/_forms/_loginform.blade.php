    ...
    <div class="login-container steps">
            ...
            {!! Form::open(array('action' => array('Auth\AuthController@login', 'redirectTo' => Request::query('redirectTo') ), 'method' => 'POST', 'onsubmit' => 'return false')) !!}
                ...
                <div class="under-email-signin clearfix">
                    ...
                    <div class="forgot-pass">
                        <a href="{{ action('Auth\PasswordController@getEmail') }}">{!! trans('passwords.forgotpassword') !!}</a>
                    </div>
                </div>
                <button type="submit" class="button button-black button-full" id="PostUserLogin">{!! trans('index.login') !!}</button>
        ...
        <div class="signup-terms">
            <div class="show-connect-forms">
                {!! trans('index.youdonthaveanaccount') !!} <a href="{{ action('Auth\AuthController@register') }}" @if(!isset($link)) rel="get:Signupform" @endif>{!! trans('index.register') !!}</a>
            </div>
        </div>
    </div>
</div>
