...
<a href="{{ url('/') }}" alt="{!! getcong('sitetitle') !!}">
<img class="site-logo" src="{!! URL::asset('assets/img/').'/'.getcong('sitelogo') !!}" alt="{!! getcong('sitetitle') !!}">
</a>
</div>
...
<li class="cats_link @if(Request::segment(1) == $categorys->name_slug || Request::segment(1) == $categorys->posturl_slug) active @endif">
<a href="{{ action('PagesController@showCategory', [$categorys->name_slug]) }}" class="biga firsg" data-type="{{ $categorys->id }}">
<i class="fa fa-{{ $categorys->icon }}"></i>
{{ $categorys->name }} 
...
@if(Auth::check())
@if(Auth::user()->usertype == 'Admin' or Auth::user()->usertype == 'Staff' or Auth::user()->verified== 1 or getcong('UserCanPost') !="false")
...
<li>
<a href="{{ action('PostsController@CreateNew', ['new'=>$categorys->posturl_slug]) }}" class="sub-item"><i class="fa fa-{{ $categorys->icon }}"></i> {!! trans('addpost.add', ['type' => ucfirst($categorys->posturl_slug)]) !!}</a>
</li>
...
<li>
<a class="sub-item" href="{{  action('Admin\DashboardController@index') }}">{!! trans('index.adminp') !!}</a>
</li>
@endif
<li>
...
<ul id="cats_{{ $cat->id }}">
<li>
<a class="biga firsg active"  data-type="{{ $cat->id }}" href="{{ action('PagesController@showCategory', [$cat->name_slug]) }}"> {{ $cat->name }}</a>
</li>
@foreach(\App\Categories::where('type', $cat->id)->orderBy('order')->limit(7)->get() as $cata)
<li>
<a class="biga" data-type="{{ $cata->id }}" href="{{ action('PagesController@showChildCategory', ['parent' => $cat->name_slug, 'catname' => $cata->name_slug]) }}"> {{ $cata->name }}</a>
</li>
@endforeach
</ul>
@endforeach
