    ...
    <div class="quiz_result_area">
        ...
        <ol>
            @foreach($entrysquizresults as $keyp => $entry)
                ...
                <li class="quiz_result" data-order="{{ $keyp }}" data-result="{{ $entry->id }}" data-link="{{ Request::url() }}" data-name="{!! trans('quiz.yougot', ['title'=> $entry->title]) !!}"  data-iname="{!! trans('quiz.igot', ['title'=> $entry->title, 'posttitle'=> $post->title]) !!}"  data-itname="{{ trans('quiz.igotfortweet', ['title'=> $entry->title]) }}" data-description="{!! strip_tags($entry->body) !!}" data-picture="{!! $entry->image > "" ? makepreview($entry->image, null, 'entries') : makepreview($post->thumb, 'b', 'posts')  !!}">

                    <h2 class="quiz_headline">
                        {!! trans('quiz.yougot', ['title'=> $entry->title]) !!}
                    </h2>
                    <div class="clear"></div>
...
<div class="clear"></div>
    ...
    <h2 class="bold share_title">{!! trans('quiz.shareresult')  !!}</h2>
