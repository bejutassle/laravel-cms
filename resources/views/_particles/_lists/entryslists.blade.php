        ...
        @else
                ...
                @if($entry->type=='image')
                    <div class="media">
                        ...
                        <small>@if(!empty($entry->source)){!! trans('index.post_source') !!} : {!! $entry->source !!}@endif</small>
                    </div>
                @endif
...
@if(isset($post->pagination) and $post->pagination!=null)
   ...
   @if($entrys->currentPage()!=1)<a href="{{ $entrys->previousPageUrl() }}" class="button button-big button-gray pull-l ">{!! trans('pagination.previous') !!}</a>@endif
       @if($entrys->currentPage()!=$entrys->lastPage())<a href="{{ $entrys->nextPageUrl() }}"  class="button button-big button-gray pull-r">{!! trans('pagination.next') !!}</a>@endif
