...
@section('head_description', htmlspecialchars(str_limit($post->body, 100), ENT_COMPAT, 'UTF-8'))
                ...
                <div class="post-content" style="margin-top:7px;background: transparent" itemscope="" itemtype="http://schema.org/Article">
                    ...
                    <div class="post">
                        <div class="post-head">
                            ...
                            @if(getcong('UserEditPosts')=='true' or Auth::user()->usertype=='Admin' or Auth::user()->usertype=='Staff' or $post->user_id == Auth::user()->id)
                   ...
                   @if($post->user->verified == 1)
                    <div class="label label-verified-sm">
                    ...
                    @endif
                        ...
                        <div class="detail">
                            ...
                            @unless($post->updated_at == $post->published_at) {!! trans('updates.and') !!}
...
<?php 
$cata = \App\Categories::
where('id', $post->category->type)
->first();
?>
<a href="{{ action('PagesController@showChildCategory', ['parent' => $cata->name_slug, 'catname' => $post->category->name_slug]) }}">{{ $post->category->name }}</a>
</div>
@endif
            ...
            <div class="sidebar">
                ...
                <div class="colheader" style="border:0;text-transform: uppercase;">
                    <h1>{!! trans('index.today') !!} {!! trans('index.top', ['type' => '<span>'.trans('index.posts').'</span>' ]) !!}</h1>
                </div>
...
@section('footer')
    ...
    <script>
         Quizzes = {
            'lang_1': '{!! trans('quiz.shareonface') !!}',
            'lang_2': '{!! trans('quiz.shareontwitter') !!}',
            'lang_3': '{!! trans('quiz.shareface') !!}',
            'lang_4': '{!! trans('quiz.sharetweet') !!}',
            'lang_5': '{!! trans('quiz.sharedone') !!}',
            'lang_6': '{!! trans('quiz.sharedonedesc') !!}'
        };
    ...
    <script>(function(d, s, id) {
            ...
            js.src = "//connect.facebook.net/{{  getcong('sitelanguage') > "" ? getcong('sitelanguage') : 'en_US' }}/sdk.js#xfbml=1{!! getcong('facebookapp') > "" ? '&appId='.getcong('facebookapp') : '' !!}&version=v2.7";
