...
<img src="{{ makepreview($userinfo->splash, 'b', 'members/splash') }}" width="100%" data-big="" alt="{{ $userinfo->username }}" class="profile-splash-img">
   <div class="profile-section">
        <a href="{{ action('UsersController@index', $userinfo->username_slug) }}"><img src="{{ makepreview($userinfo->icon, 'b', 'members/avatar') }}" width="180" height="180" data-big="" alt="{{ $userinfo->username }}" class="profile-image"></a>
    </div>

    <div class="profile-display-name">
        <a href="{{ action('UsersController@index', $userinfo->username_slug) }}" style="margin-right:5px;">{{ $userinfo->username }}</a>

       @if($userinfo->verified == 1)
        <div class="label label-verified">
        <a title="{!! trans('index.verified') !!}">
      ...
      @if($userinfo->usertype == 'Admin')
            <div class="label label-admin" style="font-size:12px;">{!! trans('updates.usertypeadmin') !!}</div>
        @elseif($userinfo->usertype == 'Staff')
            <div class="label label-staff" style="font-size:12px;">{!! trans('updates.usertypestaff') !!}</div>
        @elseif($userinfo->usertype == 'banned')
            <div class="label label-banned" style="font-size:12px;">{!! trans('updates.usertypebanned') !!}</div>
        @endif
        ...
        <div class="profile-sidebar">
            ...
            <div class="profile-section">
               ...
               <div class="profile-details">
                ...
                <a class="button button-black button-full set-button" href="{{ action('UsersController@settings', [$userinfo->username_slug]) }}">{!! trans('index.settings') !!}</a>
                @endcan
            </div>
            ...
            <div class="profile-section">
                ...
                @if($DB_PLUGIN_QUIZS == 'on')<div class="profile-stat">    <div class="profile-stat-label">   <i class="fa fa-question-circle"></i>   <span class="stat-text">{!! trans('index.total', ['type' => trans('quiz.quizzes') ]) !!}</span>    </div>    <a class="profile-stat-count" href="/profile/{{ $userinfo->username_slug }}/quizzes">{{ $quizzescount }} </a>  </div>@endif
...
@endif
            ...
            <div class="profile-section follow-images">
                <div class="profile-sidebar-label">
                    ...
                    <a class="more_follow" href="{{ action('UsersController@following', $userinfo->username_slug) }}">{!! trans('updates.allfollow', ['count' => $userinfo->following()->count()]) !!}</a>
                </div>
                @foreach($userinfo->following()->take(12)->get() as $following)

                    <a class="follow-image" href="{{ action('UsersController@following', $following['followed']['username_slug']) }}" title="{{ $following['followed']['username'] }}"><img src="{{ makepreview($following['followed']['icon'], 's', 'members/avatar') }}" width="52" height="52" alt="{{ $following['followed']['username'] }}"></a>
            ...
            <div class="profile-section follow-images">
                <div class="profile-sidebar-label">
                    ...
                    <a class="more_follow" href="{{ action('UsersController@followers', $userinfo->username_slug) }}"> {!! trans('updates.allfollow', ['count' => $userinfo->followers()->count()]) !!}</a>
                </div>
                @foreach($userinfo->followers()->take(12)->get() as $follower)

                    <a class="follow-image" href="{{ action('UsersController@followers', $follower['follower']['username_slug']) }}" title="{{ $follower['follower']['username'] }}"><img src="{{ makepreview($follower['follower']['icon'], 's', 'members/avatar') }}" width="52" height="52" alt="{{ $follower['follower']['username'] }}"></a>
