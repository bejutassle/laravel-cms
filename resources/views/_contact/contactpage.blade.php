...
@section('head_title', trans('contact.contact').' | '.getcong('sitename') )
...
<br>
            <h1 style="margin-bottom:10px">{!! trans('contact.contact') !!}</h1>
                ...
                {!!   Form::open(array('action' => 'ContactController@create', 'method' => 'POST','class' => 'form','name' => 'contactform', 'enctype' => 'multipart/form-data')) !!}
                    ...
                    <div class="form-field string  inpt">
                        <label for="label">{!! trans('contact.label') !!}</label>
                        {!! Form::select('label', $labels, ['id' => 'label']) !!}
                        <p>{!! trans('contact.labelinfo') !!}</p>
                    </div>
                @if(getcong('BuzzyContactCaptcha')=="on")
                <div class="form-field inpt">
                    <label>{!! trans('contact.areyouhuman') !!}</label>
                    <script src='https://www.google.com/recaptcha/api.js'></script>
                    <div class="g-recaptcha" data-sitekey="{{  getcong('reCaptchaKey') }}"></div>
                </div>
                @endif

                {!! Form::submit(isset($post->id) ? trans('addpost.savec') : trans('contact.send'), ['class' => 'button button-black button-full submit-button']) !!}
