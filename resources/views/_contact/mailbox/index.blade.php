...
<input type="text" name="qemail" class="form-control input-sm" placeholder="{!! trans('admin.mail_search') !!}">
</form>
...
<div class="mailbox-read-info">
    <a href="/admin/mailbox/read/{{ $mail->id }}" class="taba" style="color:#333">
       <h3 style="font-weight: 700">{{ $mail->name > "" ? $mail->name : "<{!! trans('admin.mail_no-name') !!}>" }}</h3>
        <h5>{{ $mail->email }}</h5>
    </a>
</div>
...
@if(!empty(Request::query('qemail')))
<?php $folderResult = trans('contact.search_empty', ['folder' => $catname]); ?>
@else
<?php $folderResult = trans('contact.folder_empty', ['folder' => $catname]); ?>
@endif
<h4><i class="icon fa fa-{{ $caticon }}" style="font-size:80px;margin-bottom:20px;opacity: 0.5"></i> <br>{!! utf_ucwords($folderResult) !!}</h4>
