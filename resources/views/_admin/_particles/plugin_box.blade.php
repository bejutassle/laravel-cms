<div class="box box-widget widget-user" style="margin-bottom: 30px">
    ...
    <i class="fa {{ $p_icon }}" style="font-size: 50px; cursor: pointer; color: #8a6d3b;" alt="{{ $p_name }}"></i>
    {{--
    ...
    <div class="box-footer" style="height: 60px;max-height: 60px; padding: 15px 10px 10px 15px;">
        <div class="row">
            <div class="col-sm-4 border-right">
             @unless($p_price=='soon')
                @if($p_price!='ÜCRETSİZ' and $p_status!='actived')
                    @if($p_activetihs==false)
                        <button type="button" data-item="{{ $p_code }}"  data-img="{{ $p_icon }}" data-verify="{{ $p_price!='ÜCRETSİZ' ? 'on' : 'off' }}" class="btn btn-block btn-warning btn-sm checkcodeforplugin" style="text-align: left;width:auto"><i class="fa fa-unlock" style="margin-right:10px"></i> {!! trans('admin.activate_product') !!}</button>
                    @elseif($p_actfilescont==false)
                            <span class="badge bg-white">{!! trans('admin.activated') !!}! Bu eklenti için bazı eksik dosyalar tespit edildi.<br> Lütfen eklenti için eksik olan dosyaları indirin ve gerekli kurulumu tamamlayın.</span>
                    @else
                        <button type="button" data-item="{{ $p_code }}" data-verify="{{ $p_price!='ÜCRETSİZ' ? 'on' : 'off' }}" class="btn btn-block btn-success btn-sm activebut" style="text-align: left"><i class="fa fa-download" style="margin-right:10px"></i> {!! trans('admin.install') !!}</button>
                    @endif

                    @else
                        @if($p_status=='actived')
                            <button type="button" data-item="{{ $p_code }}" data-verify="{{ $p_price!='ÜCRETSİZ' ? 'on' : 'off' }}" class="btn btn-block btn-default btn-sm activebut acthover" style="text-align: left"><span class="current show"><i class="fa fa-check" style="margin-right:10px"></i> {!! trans('admin.activated') !!}</span><span class="hover hide"><i class="fa fa-remove" style="margin-right:10px"></i> {!! trans('admin.deactivate') !!}</span></button>

                        @elseif($p_status=='notactived')
                            <button type="button" data-item="{{ $p_code }}" data-verify="{{ $p_price!='ÜCRETSİZ' ? 'on' : 'off' }}" class="btn btn-block btn-info btn-sm activebut" style="text-align: left"><i class="fa fa-download" style="margin-right:10px"></i> {!! trans('admin.install') !!}</button>
                        @else
            ...
            <div class="col-sm-4 border-right">
                @if($p_price!='ÜCRETSİZ' and $p_status!='actived' and $p_activetihs==false)
        ...
        <div class="modal modal-info" id="modal{{$p_code}}">
            <div class="modal-dialog" @if($p_code == "homepagebuilder")style="width:80%" @endif>
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">easyComment {!! trans('admin.PluginSettings') !!}</h4>
                    </div>
            ...
            @elseif($p_code == "disquscomments")
                        ...
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="{!! trans('admin.close') !!}">
                                <i class="fa fa-remove"></i></button>
                            <h4 class="modal-title">Disqus {!! trans('admin.PluginSettings') !!}</h4>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                    ...
                    @elseif($p_code == "quizzes")

                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="{!! trans('admin.close') !!}">
                                <i class="fa fa-remove"></i></button>
                            <h4 class="modal-title">Quizzes {!! trans('admin.PluginSettings') !!}</h4>
                        </div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::select('BuzzyQuizzesPopup', ['on' => 'Quizzes Special Result Popup', 'off' => 'Only BuzzFeed Style Result'], getcong('BuzzyQuizzesPopup'), ['class' => 'form-control'])  !!}
                                <p>You can change result area of Quizzes.</p>
                            </div>

                        </div>
                    @elseif($p_code == "contact")
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="{!! trans('admin.close') !!}">
                                <i class="fa fa-remove"></i></button>
                            <h4 class="modal-title">Contact {!! trans('admin.PluginSettings') !!}</h4>
                        </div>
                    ...
                    @elseif($p_code == "homepagebuilder")
                        ...
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="{!! trans('admin.close') !!}">
                                <i class="fa fa-remove"></i></button>
                            <h4 class="modal-title">Homepage Builder</h4>
                        </div>
                        <div class="modal-body">
