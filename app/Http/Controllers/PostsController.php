...
public function index($catname, $slug){
    ...
    if ($post->approve == 'no' || $post->approve == 'draft' ) {
        if(Auth::check() == true){
    ...
    Cookie::queue('PostHit'.$post->id, true, 45000);
    if (Cookie::get('PostHit'.$post->id) !== true){
        $post->hit();
    }
...
if(Auth::user()->verified == 1 AND $post->user_id == Auth::user()->id):

unset($postPermission);

$postPermission = true;

elseif(getcong('UserEditPosts') == true AND $post->user_id == Auth::user()->id):

unset($postPermission);

$postPermission = true;

elseif(Auth::user()->usertype == 'Admin' OR Auth::user()->usertype == 'Staff'):
...
$postslug = slug_valid('post', $titleslug);
...
if($inputs['datapostt']=='draft'){
    $post->approve = 'draft';
}elseif(getcong('AutoApprove')=='true' or Auth::user()->usertype == 'Staff' or Auth::user()->usertype == 'Admin'){
    $post->approve = 'yes';
}else{
    $post->approve = 'no';
}
...
if($inputs['datapostt']=='draft'){
    $post->approve = 'draft';
}elseif(getcong('AutoEdited')=='true' or Auth::user()->usertype == 'Staff' or Auth::user()->usertype == 'Admin'){
    $post->approve = 'yes';
}else{
    $post->approve = 'no';
}
...
private function resizepostimage($imgWW, $slug){
    ...
    make_path(public_path() .'/'. $tmpFilePath);
...
protected function getfailsvalidator($request, $id = NULL){
    ...
    if($inputs['type']=="quiz"){
        ...
        if($quizresultcount < 2){
            return array('status' => trans('quiz.quizerror'), 'errors' => trans('quiz.atlest2result'));
        }
        ...
        if($quizquestioncount < 1){
            return array('status' => trans('quiz.quizerror'), 'errors' => trans('quiz.atlest1question'));
        }
    }
    ...
    foreach($inputs['entrys'] as $key => $n )
    {
        ...
        if ($v->fails()) {
            ...
            if($entrytype=="quizresult"){
                ...
                return array('status' => trans('quiz.quizresulterror'), 'errors' => trans('quiz.quizresulterrors', ['numberofentry' => $keya, 'error' => $v->errors()->first()]));

            }elseif($entrytype=="quizquestion"){

                return array('status' => trans('quiz.questionerror'), 'errors' => trans('quiz.questionerrors', ['numberofentry' => $keya-$quizresultcount, 'error' => $v->errors()->first()]));

           }elseif($entrytype=="poll"){

                return array('status' => trans('quiz.questionerror'), 'errors' => trans('quiz.questionerrors', ['numberofentry' => $keya, 'error' => $v->errors()->first()]));
            ...
            if($entrytype=="quizquestion" or $entrytype=="poll"){
                ...
                if(!isset($n['answers']) or count($n['answers']) < 2){
                    return array('status' => trans('quiz.questionerror'), 'errors' => trans('quiz.questionerrors', ['numberofentry' => $key-$quizresultcount+1, 'error' => trans('quiz.atlest2answer')]));
                }
                ...
                foreach($n['answers'] as $ankey => $ann )
                {
                    ...
                    if ($qv->fails()) {
                        $keyaa=$ankey+1;
                        return array('status' => trans('quiz.answererror'), 'errors' => trans('quiz.answererrors', ['numberofentry' => $key-$quizresultcount+1, 'numberofanswer' => $keyaa, 'error' => $qv->errors()->first()]));
                    }
                }
            }
