...
public function handleSocialCallback($type, User $user){
    ...
    if(null !== $money->getEmail()){
        ...
        return redirect(action('Auth\AuthController@register'));
    }
...
public function newlogin(Request $request){
    ...
    if (Auth::attempt($credentials, $request->has('remember'))) {
        ...
        if(Auth::user()->usertype=='banned'){
            \Auth::logout();
            return array("errors" => trans('auth.user_ban'));
        }
...
public function newRegister(Request $request){
    ...
    return array('status' => trans('auth.congratz'), 'text' => trans('auth.joined'), 'url' => redirect()->back());
...
protected function mailtoregistareduser(){
    ...
    $activeurl = action('PasswordController@getActivate', ['token' => Auth::user()->remember_token]);

    try{
        $this->mail->send('emails.registered', compact('username', 'activeurl'), function($message){
