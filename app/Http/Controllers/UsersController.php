...
public function index(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('all')->latest('published_at')->paginate(15);
...
public function news(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('news')->latest('published_at')->paginate(15);
...
public function lists(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('list')->latest('published_at')->paginate(15);
...
public function quizzes(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('quiz')->latest('published_at')->paginate(15);
...
public function videos(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('video')->latest('published_at')->paginate(15);
...
public function polls(){
    ...
    $lastPosts = $this->userinfo->posts()->typesActivete()->approve('yes')->byType('poll')->latest('published_at')->paginate(15);
...
public function updatesettings(Request $request){
   ...
   $password = $inputs['password'];
    ...
    if($icon){
        ...
        make_path(public_path() .'/'. $tmpFilePath.$hardPath);
    ...
    if(getcong('UserEditUsername') == true || Auth::user()->usertype == 'Admin'){
        if($username){
    ...
    if(getcong('UserEditEmail') == true || Auth::user()->usertype == 'Admin'){
        if($email){
    ...
    return redirect(action('UsersController@settings', str_slug($username, '-')));
