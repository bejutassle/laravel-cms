...
public function __construct(Mailer $mailer){
    ...
    $this->fromemail = (getcong('ContactEmail') > '') ? getcong('ContactEmail') : getcong('siteemail');
...
public function index($type = ''){
    ...
    if($requ > ''){
        ...
        $catname = trans('admin.mail_search_for_query', ['query' => $requ]);
        $caticon = 'search';
...
public function read($id){
    ...
    if(!$lastmail){
        return  redirect(action('Admin\mailbox\ContactController@index', ['']));
    }
...
public function doaction(Request $request){
   ...
   if(!isset($requests['contacts']) or count($requests['contacts']) == 0){
       return array('type'=>'error', 'message'=> trans('admin.mail_not_selected'), 'url' => action('Admin\mailbox\ContactController@index', ['']));
   }
...
public function newmailsent(Request $request){
    ...
    if($type == 'sendit'){
        ...
        $catorurl = action('Admin\mailbox\ContactController@index', ['inbox']);
        $catormessage = 'Email successfully sent it!';
    }else if($type == 'draftsave'){
        $cator=Categories::byType('mailcat')->where('name_slug', 'drafts')->first()->id;
        $catorurl = action('Admin\mailbox\ContactController@index', ['drafts']);
        $catormessage = 'Saved as draft!';
    }
